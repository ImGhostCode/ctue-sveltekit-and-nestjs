<script lang="ts">
	...
	import FormAddSentence from '../../components/FormAddSentence.svelte';
	import { FlatToast, ToastContainer, toasts } from 'svelte-toasts';
	import FormEditSentence from '../../components/FormEditSentence.svelte';
	...
	function toggleSelected(index: number) {
		topics[index].selected = !data.topicsSentence[index].selected;
	}

	$: {
		if (data.topicsSentence) topics = data.topicsSentence;
		if (data.typesSentence) types = data.typesSentence;
	}
	...
	const handleDeleteSentence = async (id: number) => {
		const confirm = window.confirm('Bạn có chắc chắn muốn xóa câu này không?');
		if (confirm) {
			const response = await fetch(`/manage-sentence/${id}`, {
				method: 'DELETE'
			});
			const result = await response.json();
...
<dialog bind:this={myModal7} id="my_modal_" class="modal">
	<FormEditSentence {topics} {types} {currentSentence} on:cancel={handleCancel} />
</dialog>
